...
export default function Sidebar({ open, onClose, userName, canManageSettings }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 flex flex-col bg-slate-900/40 backdrop-blur-xl border-r border-slate-700/50 transition-transform duration-300 ease-in-out md:static md:translate-x-0 ${
          open ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="h-16 flex items-center px-6 border-b border-slate-700/50">
          <h1 className="text-lg font-bold text-slate-100">SufalPhysicsForum</h1>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-300 ease-in-out ${
                  isActive
                    ? 'bg-blue-600/30 text-yellow-300 shadow-lg shadow-blue-900/30'
